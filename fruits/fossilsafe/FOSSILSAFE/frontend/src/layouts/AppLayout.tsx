...
export default function AppLayout() {
    ...
    // Check system stats
    useEffect(() => {
        const fetchStats = async (includeExtended = false) => {
            if (isDemoMode) {
                setStats([
            ...
            try {
                ...
                const [sysRes, jobRes] = baseResults.map(r =>
                    r.status === 'fulfilled' ? r.value : { success: false }
                ) as any[];

                let tapeRes: any = { success: false };
                let driveRes: any = { success: false };
                ...
                if (includeExtended) {
                    ...
                    [tapeRes, driveRes] = extendedResults.map(r =>
                        r.status === 'fulfilled' ? r.value : { success: false }
                    ) as any[];
                }
                ...
                const getDrives = () => {
                    ...
                    if (!driveRes.success || !driveRes.data) return "---";
                    const drives = Object.values(driveRes.data as Record<string, { status?: string }>);
                    if (drives.length === 0) return "---";
                    const unhealthy = drives.filter((d: any) => d.status && d.status !== 'healthy').length;
                    return unhealthy > 0 ? `${unhealthy}/${drives.length} DEGRADED` : `${drives.length} HEALTHY`;
                };

                const getStorage = () => {
                    if (!includeExtended) return "---";
                    if (!tapeRes.success || !tapeRes.data) return "---";
                    const used = tapeRes.data.used_capacity_bytes || 0;
                    const tb = (used / (1024 ** 4)).toFixed(1);
                    return `${tb} TB`;
                };

                const driveHealth = includeExtended && driveRes.success && driveRes.data
                    ? Object.values(driveRes.data as Record<string, { status?: string }>)
                    : [];
                const needsAttention = driveHealth.some((d: any) => d.status && d.status !== 'healthy');
                ...
                const newStats = [
                    ...
                    {
                        label: 'TAPES',
                        value: includeExtended && tapeRes.success && tapeRes.data
                            ? `${tapeRes.data.tapes_online ?? 0}/${tapeRes.data.total_slots ?? 0}`
                            : "---",
                        icon: 'hard_drive'
                    },
                    ...
                    {
                        label: 'MAINTENANCE',
                        value: includeExtended ? (needsAttention ? 'ATTENTION' : 'OPTIMAL') : '---',
                        icon: 'cleaning_services',
                        urgent: includeExtended && needsAttention
                    }
                ];
