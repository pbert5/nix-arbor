...
const MOCK_EQUIPMENT = {
    drives: [
        { id: 'DRV-01', type: 'LTO-8 SAS', status: 'Online', activity: 'Writing', speed: '285 MB/s', tape: 'LTO-8-0042', health: 98, healthStatus: 'healthy' },
        { id: 'DRV-02', type: 'LTO-8 SAS', status: 'Online', activity: 'Idle', speed: '0 MB/s', tape: 'None', health: 100, healthStatus: 'healthy' },
        { id: 'DRV-03', type: 'LTO-7 SAS', status: 'Offline', activity: 'Disabled', speed: '0 MB/s', tape: 'None', health: 85, healthStatus: 'degraded' },
    ],
    libraries: [
...
function getDriveIndex(entry: any, fallback = 0) {
    const candidates = [entry?.drive_index, entry?.drive_id, entry?.drive_source_slot, entry?.id];
        ...
        if (typeof candidate === 'string') {
            ...
            if (matches?.length) {
                ...
                if (Number.isFinite(parsed)) {
                    return candidate.startsWith('DRV-') ? Math.max(0, parsed - 1) : parsed;
                }
            }
        }
    }
...
function formatDriveId(index: number) {
    return `DRV-${String(index + 1).padStart(2, '0')}`;
}

function normalizeGenerationLabel(value?: string | null) {
    if (!value) return 'Configured Tape Drive';

    const raw = String(value).trim().toUpperCase().replace(/\s+/g, '');
    if (/^LTO-\d+$/.test(raw)) return `${raw} Drive`;
    if (/^LTO\d+$/.test(raw)) return `LTO-${raw.slice(3)} Drive`;
    if (/^LT\d+$/.test(raw)) return `LTO-${raw.slice(2)} Drive`;
    if (/^L\d+$/.test(raw)) return `LTO-${raw.slice(1)} Drive`;
    if (raw === 'CLEANING') return 'Cleaning Drive';

    return `${String(value).trim()} Drive`;
}

function getDrivePresentation(entry: any) {
    const rawStatus = String(entry?.status || '').toLowerCase();
    const hasTape = !!entry?.barcode;

    if (rawStatus === 'error') {
        return { status: 'Error', activity: 'Attention', speed: '---' };
    }

    if (rawStatus === 'offline') {
        return { status: 'Offline', activity: 'Unavailable', speed: '0 MB/s' };
    }

    if (rawStatus === 'erasing') {
        return { status: 'Online', activity: 'Erasing', speed: 'ACTIVE' };
    }

    if (rawStatus === 'wiping') {
        return { status: 'Online', activity: 'Wiping', speed: 'ACTIVE' };
    }

    if (rawStatus === 'initializing') {
        return { status: 'Online', activity: 'Initializing', speed: 'ACTIVE' };
    }

    if (rawStatus === 'busy' || rawStatus === 'in_use') {
        return { status: 'Online', activity: 'Active', speed: 'ACTIVE' };
    }

    if (rawStatus === 'loaded' || hasTape) {
        return { status: 'Online', activity: 'Mounted', speed: '0 MB/s' };
    }

    return { status: 'Online', activity: 'Idle', speed: '0 MB/s' };
}

function buildPredictiveHealthByIndex(predictiveHealth: Record<string, any>) {
    const byIndex = new Map<number, any>();

    Object.entries(predictiveHealth || {}).forEach(([key, value]) => {
        const index = getDriveIndex({ drive_id: value?.drive_id, id: key }, -1);
        if (index >= 0) {
...
function createEmptyDriveEntry(index: number) {
    return {
        barcode: '',
        alias: '',
        generation: undefined,
        status: 'empty',
        location: `Drive ${index}`,
        location_type: 'drive',
        drive_index: index,
        drive_full: false,
        is_placeholder: true,
...
export default function EquipmentPage() {
    ...
    const [predictiveHealth, setPredictiveHealth] = useState<Record<string, any>>({});
    const [loading, setLoading] = useState(!isDemoMode);
    const [error, setError] = useState<string | null>(null);
    const [scanning, setScanning] = useState(false);
    const hasLoadedOnceRef = useRef(isDemoMode);
    ...
    const drives = useMemo(() => {
        if (isDemoMode) {
            return MOCK_EQUIPMENT.drives.map((drive, index) => ({ ...drive, index }));
        }

        const healthByIndex = buildPredictiveHealthByIndex(predictiveHealth);

        return inventory
            .filter(entry => entry.location_type === 'drive')
            .sort((a, b) => getDriveIndex(a) - getDriveIndex(b))
            .map((entry, fallbackIndex) => {
                const index = getDriveIndex(entry, fallbackIndex);
                const presentation = getDrivePresentation(entry);
                const health = healthByIndex.get(index);

                return {
                    index,
                    id: formatDriveId(index),
                    type: normalizeGenerationLabel(entry?.generation),
                    status: presentation.status,
                    activity: presentation.activity,
                    speed: presentation.speed,
                    tape: entry?.barcode || 'None',
                    health: typeof health?.score === 'number' ? Math.round(health.score) : null,
                    healthStatus: health?.status || 'unknown',
                };
            });
    }, [inventory, isDemoMode, predictiveHealth]);

    const libraries = useMemo(() => {
        if (isDemoMode) {
            return MOCK_EQUIPMENT.libraries.map(lib => ({ ...lib, utilizationPercent: 60 }));
        }

        const occupiedSlots = inventory.filter(entry => entry.location_type === 'slot' && entry.barcode).length;
        const totalSlots = systemStats?.total_slots || occupiedSlots;
        const utilizationPercent = totalSlots > 0 ? Math.min(100, Math.round((occupiedSlots / totalSlots) * 100)) : 0;

        return [{
            id: 'LIB-DEFAULT',
            name: 'Integrated Library',
            status: systemStats ? 'Synchronized' : 'Cached',
            slots: `${occupiedSlots} / ${Math.max(totalSlots, occupiedSlots)}`,
            drives: drives.length,
            utilizationPercent,
        }];
    }, [drives.length, inventory, isDemoMode, systemStats]);
    ...
    useEffect(() => {
        ...
        const loadInventory = async (showLoader = false) => {
            ...
            if (tapesRes.success && tapesRes.data) {
                setInventory(tapesRes.data.tapes);
                setError(null);
                hasLoadedOnceRef.current = true;
            } else if (!hasLoadedOnceRef.current) {
                setError(tapesRes.error || 'Failed to sync with cached library inventory');
            }
        ...
        const loadAuxiliaryData = async () => {
            ...
            if (healthRes.status === 'fulfilled' && healthRes.value.success && healthRes.value.data) {
                setPredictiveHealth(healthRes.value.data);
            }
        };

        const refreshVisibleTopology = (showLoader = false) => {
            if (document.visibilityState === 'hidden') {
                return;
            }

            void loadInventory(showLoader);
            void loadAuxiliaryData();
        };

        refreshVisibleTopology(true);

        const interval = setInterval(() => refreshVisibleTopology(false), 60000);
        const handleVisibilityChange = () => {
            if (document.visibilityState === 'visible') {
                refreshVisibleTopology(false);
            }
        };
    ...
    const handleUnmount = async (driveId: string) => {
        ...
        const res = await api.unloadTape(drive.index);
        if (res.success) {
            setInventory(prev => {
                const next = prev.map(entry => {
                    if (entry.location_type !== 'drive' || getDriveIndex(entry, -1) !== drive.index) {
                        return entry;
                    }

                    return {
                        ...entry,
                        ...createEmptyDriveEntry(drive.index),
                    };
                });

                if (next.some(entry => entry.location_type === 'drive' && getDriveIndex(entry, -1) === drive.index)) {
                    return next;
                }

                return [...next, createEmptyDriveEntry(drive.index)];
            });
        } else {
                                        ...
                                        <div className="flex gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-3 mt-2">
                                                    ...
                                                    >
                                                        Health: {typeof drive.health === 'number' ? `${drive.health}%` : '--'}
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-6 bg-black/30 rounded p-6 border border-[#27272a]">
                                        <div>
                                            ...
                                            <div className="w-full h-1 bg-[#27272a] rounded-full mt-3 overflow-hidden">
                                                <div
                                                    className="h-full rounded-full bg-primary shadow-[0_0_8px_rgba(25,230,100,0.5)]"
                                                    style={{ width: `${lib.utilizationPercent ?? 0}%` }}
                                                ></div>
                                            </div>
                                        </div>
                                        <div>
                ...
                onConfirm={() => {
                    if (confirmModal.type === 'unmount') {
                        if (confirmModal.driveId) {
                            handleUnmount(confirmModal.driveId);
                        }
                    } else if (confirmModal.type === 'lock') {
                        if (confirmModal.tape) {
                            handleLockTape(confirmModal.tape);
                        }
                    }
                }}
