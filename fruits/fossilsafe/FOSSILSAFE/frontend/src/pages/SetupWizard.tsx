...
export default function SetupWizard({ onComplete }: SetupWizardProps) {
    ...
    const checkStatus = async () => {
        try {
            ...
            setNative2FAEnabled(!!res.data?.native_2fa_enabled)
        } catch (e) {
    ...
    const checkTapeStep = async () => {
        setIsLoading(true)
        try {
            const res = await api.getSetupTapeStatus()
    ...
    const handleStartTapeInit = async () => {
        setError('')
        try {
            const res = await api.startSetupTapeInit()
            if (res.success) {
                pollTapeInit()
            } else {
    ...
    const pollTapeInit = async () => {
        const interval = setInterval(async () => {
            try {
                const res = await api.getSetupTapeInitStatus()
                if (res.success && res.data) {
                ...
                <div className="bg-[#121214] border border-[#27272a] rounded-lg overflow-hidden shadow-2xl">
                    ...
                    <div className="h-1 bg-[#27272a] flex">
                        {['welcome', 'secure-check', 'admin', '2fa', 'tape-init', 'complete'].map((s) => {
                            ...
                            if ((isRecoveryMode || !native2FAEnabled) && s === '2fa') return null
