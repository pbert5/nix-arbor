...
const SUPPORTED_MEDIA_TYPES = ['LTO-4', 'LTO-5', 'LTO-6', 'LTO-7', 'LTO-8', 'LTO-9'];

function normalizeMediaType(value?: string | null, isCleaning = false) {
    ...
    if (raw === 'CLEANING' || raw.startsWith('CLN')) return 'Cleaning';

    if (/^LT\d+$/.test(raw)) {
        return `LTO-${raw.slice(2)}`;
    }

    if (/^LTO\d+$/.test(raw)) {
        return `LTO-${raw.slice(3)}`;
    }

    if (/^LTO-\d+$/.test(raw)) {
        return `LTO-${raw.slice(4)}`;
    }

    if (/^L\d+$/.test(raw)) {
        return `LTO-${raw.slice(1)}`;
    }

    return String(value).trim();
}

function getTapeStatusFilterValue(status?: string) {
    if (!status) return 'other';
    if (status.startsWith('Drive')) return 'in-drive';
...
export default function TapesPage() {
    ...
    const mapTapeFromApi = (t: any) => {
        const isCleaningTape = t.is_cleaning_tape || t.barcode?.startsWith('CLN');
        const generation = normalizeMediaType(t.generation, isCleaningTape);
        ...
        return {
            ...
            location: t.location_type === 'drive'
                ? `Drive ${(t as any).drive_id ?? (t as any).drive_source_slot ?? (t as any).drive_index ?? 0}`
                : t.location_type === 'mailslot'
                    ? `Mailslot ${String(t.slot_number || (t as any).slot || 0).padStart(3, '0')}`
                    : `Slot ${String(t.slot_number || (t as any).slot || 0).padStart(3, '0')}`,
            location_type: t.location_type,
            drive_id: (t as any).drive_id ?? (t as any).drive_source_slot ?? (t as any).drive_index ?? 0,
            status: t.location_type === 'drive'
                ? `Drive ${(t as any).drive_id ?? (t as any).drive_source_slot ?? (t as any).drive_index ?? 0}`
                : t.status === 'available'
                    ? 'Online'
                    : t.status === 'in_use'
            ...
            is_cleaning: isCleaningTape,
            is_cleaning_tape: isCleaningTape,
            cleaning_lives: t.cleaning_remaining_uses,
            read_only: t.read_only,
            writable: t.writable,
            write_block_reason: t.write_block_reason,
        };
    };
    ...
    const availableMediaTypes = useMemo(() => {
        const discovered = new Set(
            tapes
                .map(tape => normalizeMediaType(tape.generation || tape.type, tape.is_cleaning || tape.is_cleaning_tape))
                .filter(Boolean)
        );

        const extras = Array.from(discovered)
            .filter(type => !SUPPORTED_MEDIA_TYPES.includes(type))
            .sort((a, b) => a.localeCompare(b));

        return [...SUPPORTED_MEDIA_TYPES, ...extras];
    }, [tapes]);
    ...
    const filteredTapes = useMemo(() => {
        ...
        return tapes.filter(t => {
            if (statusFilter !== 'all' && getTapeStatusFilterValue(t.status) !== statusFilter) {
                return false;
            }

            if (mediaTypeFilter !== 'all') {
                const tapeMediaType = normalizeMediaType(t.generation || t.type, t.is_cleaning || t.is_cleaning_tape);
                if (tapeMediaType !== mediaTypeFilter) {
                    return false;
                }
            }
            ...
            const matchBarcode = (t.barcode?.toLowerCase() || "").includes(query);
            const matchAlias = (t.alias?.toLowerCase() || "").includes(query);
            return matchBarcode || matchAlias;
        });
    }, [mediaTypeFilter, searchQuery, statusFilter, tapes]);
    ...
    const totalCapacity = tapes.reduce((sum, t) => {
        const parsed = parseFloat(String(t.total || '0').replace('TB', ''));
    ...
    const handleScanLibrary = async () => {
        ...
        try {
            ...
            if (res.success && res.data) {
                const mapped = res.data.tapes.map(mapTapeFromApi);
                setTapes(mapped);
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <div className="flex shrink-0 items-center justify-between border-b border-[#27272a] bg-[#121214] px-6 py-3 z-10">
                    <div className="flex items-center gap-3">
                        <select
                            className="h-8 rounded-md border border-[#27272a] bg-[#18181b] px-3 text-[10px] font-bold uppercase tracking-wider text-[#e4e4e7] focus:border-primary focus:ring-0 outline-none"
                            value={statusFilter}
                            onChange={(e) => {
                        ...
                        <button
                            onClick={handleScanLibrary}
                            disabled={scanning || isDemoMode}
                            className="text-[10px] font-bold text-primary hover:underline flex items-center gap-1 uppercase tracking-wider disabled:opacity-50 disabled:no-underline"
                        >
                ...
                <div className="flex-1 overflow-auto bg-[#09090b] custom-scrollbar">
                    <table className="w-full text-left text-xs border-collapse">
                        ...
                        <tbody className="divide-y divide-[#27272a] bg-[#09090b]">
                            {paginatedTapes.length === 0 ? (
                                <tr>
                                    <td colSpan={9} className="px-6 py-16 text-center">
                                        <div className="flex flex-col items-center gap-2 text-[#71717a]">
                                            <Inbox className="size-8" />
                                            <span className="text-xs font-mono uppercase tracking-widest">No tapes match the current filters</span>
                                        </div>
                                    </td>
                                </tr>
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 text-[#e4e4e7] font-bold tracking-wide uppercase text-[10px]">
                                        <div className="flex flex-col gap-1">
                                            <span>{tape.alias}</span>
                                            <div className="flex items-center gap-2">
                                                <span className="text-[9px] font-mono uppercase text-[#71717a]">{tape.generation || tape.type || 'Unknown'}</span>
                                                {tape.read_only && (
                                                    <span className="inline-flex items-center rounded-full border border-amber-500/20 bg-amber-500/10 px-2 py-0.5 text-[8px] font-bold uppercase tracking-wider text-amber-400">
                                                        Read Only
                                                    </span>
                                                )}
                                            </div>
