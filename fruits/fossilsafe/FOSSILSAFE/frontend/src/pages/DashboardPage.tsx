...
export default function DashboardPage() {
    ...
    const buildFallbackStatsFromTapes = (tapesList: Tape[]): SystemStats => ({
        cache_disk_percent: 0,
        cpu_percent: 0,
        ram_percent: 0,
        total_capacity_bytes: tapesList.reduce((total, tape) => total + (tape.capacity_bytes ?? 0), 0),
        used_capacity_bytes: tapesList.reduce((total, tape) => total + (tape.used_bytes ?? 0), 0),
        tapes_online: tapesList.filter(tape => tape.status !== 'offline').length,
    ...
    const fetchDashboardData = async ({ showLoader = false }: { showLoader?: boolean } = {}) => {
        ...
        const shouldBlockRender = showLoader && !hasLoadedOnceRef.current;
        ...
        let currentTapes: Tape[] = tapes;
        const isCurrentRequest = () => requestId === requestIdRef.current;

        const [jobsRes, tapesRes] = await Promise.allSettled([
            api.getJobs(10),
            api.getTapes(),
        ]);
        ...
        const resolvedJobs = jobsRes.status === 'fulfilled'
            ? jobsRes.value
            : { success: false, error: 'Failed to load jobs' };

        if (resolvedJobs.success && resolvedJobs.data) {
            setJobs(resolvedJobs.data.jobs);
            setActiveJob(resolvedJobs.data.jobs.find(j => j.status === 'running') || null);
            telemetrySuccess = true;
        } else {
            newErrors.push('Failed to load jobs');
        }

        const resolvedTapes = tapesRes.status === 'fulfilled'
            ? tapesRes.value
            : { success: false, error: 'Failed to load tapes' };

        if (resolvedTapes.success && resolvedTapes.data) {
            currentTapes = resolvedTapes.data.tapes;
            setTapes(currentTapes);
            if (!hasLoadedOnceRef.current || !stats) {
                setStats(buildFallbackStatsFromTapes(currentTapes));
            }
            telemetrySuccess = true;
        } else {
        ...
        telemetrySuccess = telemetrySuccess || currentTapes.length > 0;
        ...
        ]).then(([drivesRes, statsRes, healthRes]) => {
            if (!isCurrentRequest()) {
                return;
            }

            const extendedErrors = [...newErrors];
            let nextTelemetrySuccess = telemetrySuccess;

            const resolvedDrives = drivesRes.status === 'fulfilled'
                ? drivesRes.value
                : { success: false, error: 'Failed to load drive status' };

            if (resolvedDrives.success && resolvedDrives.data?.drives) {
                setDrives(resolvedDrives.data.drives.map(d => ({
                    id: d.id,
                    type: d.type,
                    status: d.status as any,
                    current_tape: d.current_tape
                })));
                nextTelemetrySuccess = true;
            } else if (!extendedErrors.includes('Failed to load drive status')) {
                extendedErrors.push('Failed to load drive status');
            }

            const resolvedStats = statsRes.status === 'fulfilled'
                ? statsRes.value
                : { success: false, error: 'Failed to load system stats' };

            if (resolvedStats.success && resolvedStats.data) {
                const apiData = resolvedStats.data as any;
                ...
                nextTelemetrySuccess = nextTelemetrySuccess || totalCap > 0 || currentTapes.length > 0;
            } else if (!currentTapes.length && !extendedErrors.includes('Failed to load system stats')) {
                extendedErrors.push('Failed to load system stats');
            }

            const resolvedHealth = healthRes.status === 'fulfilled'
                ? healthRes.value
                : { success: false, error: 'Failed to load predictive drive health' };

            if (resolvedHealth.success && resolvedHealth.data) {
                setDriveHealth(resolvedHealth.data || {});
            }

            setIsLive(nextTelemetrySuccess);
            setErrors(extendedErrors);
        }).catch(() => {
            // Background enrichments should never block the dashboard shell.
        });
    };
