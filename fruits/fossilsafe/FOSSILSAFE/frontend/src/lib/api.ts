...
export class ApiClient {
    ...
    private async request<T>(
        ...
        try {
            ...
            const contentType = typeof res.headers?.get === 'function'
                ...
                : ''
            const canReadJson = contentType.includes('application/json') || typeof (res as { json?: unknown }).json === 'function'
    ...
    async getSetupStatus() {
        return this.request<{ setup_required: boolean; setup_mode?: 'secure' | 'relaxed'; native_2fa_enabled?: boolean }>('/api/auth/setup-status')
    }

    async getSetupTapeStatus() {
        return this.request<{
            has_library: boolean
            tapes: Tape[]
            count: number
            ...
            skipped_read_only?: Tape[]
        }>('/api/setup/tape-status')
    }

    async startSetupTapeInit() {
        return this.request<{ message: string }>('/api/setup/tape-init', {
            method: 'POST'
        })
    }

    async getSetupTapeInitStatus() {
        return this.request<{
            running: boolean
            current: number
            total: number
