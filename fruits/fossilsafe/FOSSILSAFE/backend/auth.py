...
def is_native_2fa_enabled() -> bool:
    config = load_config()
    auth_config = (config.get("auth", {}) or {})
    return bool(auth_config.get("native_2fa_enabled", True))
    ...
    def list_users(self) -> list:
        ...
        results = self.db.execute(
            ...
            SELECT
                ...
                created_at,
                last_login,
                CASE WHEN COALESCE(totp_secret, '') != '' THEN 1 ELSE 0 END AS has_2fa
            FROM users
            """
        ).fetchall()
        users = []
        for row in results:
            ...
            user['has_2fa'] = bool(user.get('has_2fa', 0))
            users.append(user)
        return users
...
def _auth_bypass_enabled() -> bool:
    """Allow local/dev operation without session or API-key auth."""
    return os.environ.get("FOSSILSAFE_REQUIRE_API_KEY", "true").lower() in ("0", "false", "no")


def _ensure_bypass_session() -> None:
    """Install a synthetic admin session when auth is explicitly disabled."""
    if getattr(g, 'session', None):
        return
    now = datetime.now()
    g.session = Session(
        token="dev_auth_bypass",
        user_id=0,
        role="admin",
        has_2fa=True,
        created_at=now,
        expires_at=now + timedelta(hours=AuthManager.SESSION_DURATION_HOURS),
    )
...
def require_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if is_native_2fa_enabled() and user and user.totp_secret and not session.has_2fa:
            return jsonify({
                'success': False, 
