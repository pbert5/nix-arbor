...
def _normalize_local_api_host(bind: Optional[str]) -> str:
    """Resolve a backend bind address to a loopback-safe local API host."""
    host = (bind or '').strip()
    if host in ('', '0.0.0.0', '::', '[::]'):
        return '127.0.0.1'
    return host
...
def _build_local_api_base_url(bind: Optional[str] = None, port: Optional[str] = None) -> str:
    """Build the local backend base URL used by tape change helper scripts."""
    resolved_host = _normalize_local_api_host(bind or os.environ.get('FOSSILSAFE_BACKEND_BIND'))
    resolved_port = str(port or os.environ.get('FOSSILSAFE_BACKEND_PORT') or '5000').strip() or '5000'
...
class TapeSpanningManager:
    ...
    def __init__(self, tape_controller, socketio=None, log_callback=None, api_base_url: Optional[str] = None):
        self.tape_controller = tape_controller
        self.socketio = socketio
        self.log_callback = log_callback
        self.api_base_url = (api_base_url or _build_local_api_base_url()).rstrip('/')
...
curl -X POST {self.api_base_url}/api/spanning/{job_id}/request-change \\
...
while [ $WAITED -lt $MAX_WAIT ]; do
    STATUS=$(curl -s {self.api_base_url}/api/spanning/{job_id}/status | grep -o '"state":"[^"]*"' | cut -d'"' -f4)
