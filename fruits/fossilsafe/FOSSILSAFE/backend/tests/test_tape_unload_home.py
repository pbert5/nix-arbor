...
from backend.exceptions import TapeUnloadError
...
class TapeUnloadHomeSlotTests(unittest.TestCase):
    ...
    def test_unload_raises_when_mtx_command_fails(self):
        controller = TapeLibraryController(
            device="/dev/nst0",
            changer="/dev/sg1",
            config={"tape": {"mtx_unload_order": "slot_drive"}},
        ...
        controller._set_mounted_tape(0, "TAPE1")
        controller._is_ltfs_mounted = mock.Mock(return_value=False)
        controller._confirm_drive_unloaded = mock.Mock(return_value=False)
        controller._run_mtx_command = mock.Mock(
            return_value=CommandResult(
                command=[],
                stdout="",
                stderr="illegal <drive-number> argument '5' to 'unload' command",
                returncode=1,
                duration=0.1,
                timed_out=False,
            )
        ...
        with self.assertRaises(TapeUnloadError):
            controller.unload_tape(drive=0)

        self.assertEqual(controller._get_mounted_tape(0), "TAPE1")
