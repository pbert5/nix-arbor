...
class TapeControllerParsingTests(unittest.TestCase):
    ...
    def test_parse_generation_supports_lto4(self):
        self.assertEqual(self.controller._parse_generation("T00001L4"), "LTO-4")
        self.assertEqual(self.controller.get_capacity_for_generation("LTO-4"), "800 GB")
    ...
    def test_load_tape_decrements_busy_on_exception(self):
        self.controller.scan_library = mock.Mock(return_value=[{"barcode": "ABC123", "slot": 1}])
    ...
    def test_load_tape_unloads_full_drive(self):
        ...
        self.controller.scan_library = mock.Mock(return_value=[{"barcode": "NEW123", "slot": 1}])
    ...
    def test_load_tape_uses_fast_scan_for_slot_lookup(self):
        self.controller.poll_tape_alerts = mock.Mock(return_value=[])
        self.controller._refresh_inventory_cache = mock.Mock()
        self.controller.get_drive_status = mock.Mock(return_value={
            "in_use": False,
            "loaded_tape": None,
            "source_slot": None,
        ...
        self.controller.scan_library = mock.Mock(return_value=[{"barcode": "FAST123", "slot": 7}])
        self.controller._run_mtx_command = mock.Mock(
            return_value=CommandResult(
                command=[],
                stdout="",
        ...
        self.controller.load_tape("FAST123", drive=0)

        self.controller.scan_library.assert_called_once_with(mode="fast")
    ...
    def test_unmount_ltfs_prefers_fusermount_without_sudo_prompt(self):
        self.controller._get_device_sg = mock.Mock(return_value=None)
        self.controller._is_ltfs_mounted = mock.Mock(return_value=True)
        self.controller._find_fusermount = mock.Mock(return_value="/usr/bin/fusermount3")
        self.controller.command_runner = mock.Mock()
        self.controller.command_runner.run.return_value = CommandResult(
            command=["/usr/bin/fusermount3", "-u", "/mnt/test"],
            stdout="",
            stderr="",
            returncode=0,
            duration=0.1,
        ...
        self.assertTrue(self.controller.unmount_ltfs(mount_point="/mnt/test", drive=0))

        first_call = self.controller.command_runner.run.call_args_list[0]
        self.assertEqual(first_call.args[0], ["/usr/bin/fusermount3", "-u", "/mnt/test"])
