...
class TestRBACSystem(unittest.TestCase):
    ...
    def test_list_users_reports_2fa_state(self):
        """User listing should include whether a user has native 2FA enrolled."""
        admin = self.auth.get_user("admin_user")
        self.assertIsNotNone(admin)

        self.db.execute(
            "UPDATE users SET totp_secret = ? WHERE id = ?",
            ("BASE32SECRET", admin.id)
        )
        self.db.commit()

        users = {user['username']: user for user in self.auth.list_users()}
        self.assertTrue(users["admin_user"]["has_2fa"])
        self.assertFalse(users["viewer_user"]["has_2fa"])
