...
class TestLibraryManagerIntegration:
    ...
    def test_find_controller_for_tape_single_library_skips_inventory_scan(self, mock_db):
        """Single-library setups should not deep-scan inventory just to resolve a tape controller."""
        manager = LibraryManager(mock_db)

        controller = MagicMock(spec=TapeLibraryController)
        manager.controllers = {'default': controller}
        manager.default_library_id = 'default'

        assert manager.find_controller_for_tape('TAPE_A') == controller
    ...
    def test_tape_service_scan_does_not_persist_mail_slot_autodetect(self, mock_db):
        """Auto-detected mail slots should not rewrite declarative config files."""
        manager = LibraryManager(mock_db)
        c1 = MagicMock(spec=TapeLibraryController)
        c1.library_id = 'lib_1'
        c1.scan_library.return_value = [
            {'barcode': 'TAPE_A', 'slot': 1, 'location_type': 'slot'},
            {'barcode': '', 'slot': 24, 'location_type': 'mailslot', 'status': 'empty'},
        ]

        manager.controllers = {'lib_1': c1}
        manager.default_library_id = 'lib_1'
        mock_db.get_tape_aliases.return_value = {}
        mock_db.get_active_jobs.return_value = []
        mock_db.get_tape_inventory.return_value = []

        service = TapeService(mock_db, library_manager=manager)

        with patch('backend.config_store.update_config') as mock_update_config:
            service.scan_library_and_update(mode='fast', library_id=None)

        mock_update_config.assert_not_called()
        scanned_tapes = mock_db.update_tape_inventory.call_args[0][0]
        assert any(tape.get('location_type') == 'mailslot' for tape in scanned_tapes)
