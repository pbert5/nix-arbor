...
class MockSmbClient:
    ...
    def _normalize_path(self, path):
        return path.rstrip('/') if isinstance(path, str) and path.endswith('/') else path
    def mount_share(self, share_path=None, **kwargs):
        share = share_path or kwargs.get('share')
        return share if not share.startswith('//') else "/tmp/mock_mount"
    def unmount_share(self, share_path=None, mount_point=None, **kwargs):
        pass
    def list_files(self, path, **kwargs):
        ...
        return results
    def download_file(self, share_path=None, username=None, password=None, remote_file=None, local_file=None, domain=None, **kwargs):
        src = os.path.join(share_path, remote_file)
        print(f"DEBUG_MOCK: Downloading {src} to {local_file}")
