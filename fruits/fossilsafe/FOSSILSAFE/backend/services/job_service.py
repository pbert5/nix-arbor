...
from backend.tape.policy import filter_writable_tapes, get_read_only_generations
...
class JobService:
    ...
    def create_job(self, data: Dict) -> Tuple[bool, Any]:
        ...
        if job_type == 'backup':
            ...
            selected_tapes = data.get('tapes') or []
            if selected_tapes:
                readonly = set(get_read_only_generations())
                blocked = []
                for tape_barcode in selected_tapes:
                    tape = self.db.get_tape(tape_barcode) or {'barcode': tape_barcode}
                    if (tape.get('generation') or 'Unknown') in readonly:
                        blocked.append(tape_barcode)
                if blocked:
                    return False, f"Legacy read-only tapes cannot be used for backup jobs: {', '.join(blocked)}"
    ...
    @staticmethod
    def _estimate_tapes_needed(total_size: int, tape_capacity: float) -> int:
        """Return the number of tapes required for a byte count and capacity."""
        if total_size <= 0 or tape_capacity <= 0:
            return 1

        capacity_bytes = int(tape_capacity)
        if capacity_bytes <= 0:
            return 1

        return max(1, (int(total_size) + capacity_bytes - 1) // capacity_bytes)
    ...
    def dry_run(self, data: Dict) -> Tuple[bool, Any]:
        ...
        if source_id:
            ...
            if source:
                ...
                if source_type == 'nfs' and not smb_path:
                    nfs_server = source.get('nfs_server') or ''
                    nfs_export = source.get('nfs_export') or ''
                    smb_path = f"{nfs_server}:{nfs_export}".strip(':')
        ...
        try:
            ...
            available_tapes = filter_writable_tapes(self.db.get_available_tapes())
            generation_capacities = {
                'LTO-4': 0.8 * 1024**4, 'LTO-5': 1.5 * 1024**4, 'LTO-6': 2.5 * 1024**4, 'LTO-7': 6.0 * 1024**4,
            ...
            result['estimates']['tapes_needed_native'] = self._estimate_tapes_needed(total_size, tape_capacity)
            result['estimates']['tapes_needed_compressed'] = self._estimate_tapes_needed(total_size, compressed_capacity)
            
            generation_speeds = {
                'LTO-4': 120 * 1024**2, 'LTO-5': 140 * 1024**2, 'LTO-6': 160 * 1024**2, 'LTO-7': 300 * 1024**2,
