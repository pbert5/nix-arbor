...
from backend.tape.policy import annotate_tape, get_read_only_generations
...
class TapeService:
    ...
    def get_read_only_generations(self) -> List[str]:
        return get_read_only_generations()
    ...
    def apply_active_tape_job_state(self, tapes: List[Dict], active_jobs: List[Dict]) -> List[Dict]:
        ...
        readonly = self.get_read_only_generations()
        return [annotate_tape(tape, readonly) for tape in result]
