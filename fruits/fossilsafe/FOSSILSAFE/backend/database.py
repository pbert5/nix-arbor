...
def _resolve_database_path(db_path: str) -> str:
    """Resolve relative test/dev database paths into a writable location."""
    if not db_path or db_path == ':memory:':
        return db_path

    resolved = os.fspath(db_path)
    if not os.path.isabs(resolved):
        candidate = os.path.abspath(os.path.expanduser(resolved))
        writable_parent = os.path.dirname(candidate) or os.getcwd()
        ...
        if writable_parent and os.access(writable_parent, os.W_OK):
            ...
            base_dir = os.environ.get('FOSSILSAFE_DATA_DIR') or tempfile.gettempdir()
...
class ConnectionPool:
    ...
    def __init__(self, db_path: str, pool_size: int = 100, timeout: int = 30):
...
class Database:
    ...
    def __init__(self, db_path: str = 'lto_backup.db', pool_size: int = 100, pool_timeout: int = 30):
        self.db_path = _resolve_database_path(db_path)
        self.pool = ConnectionPool(self.db_path, pool_size, pool_timeout)
        self._lock = threading.Lock()
    ...
    def add_diagnostics_report(self, *args, **kwargs) -> int:
        """Add a new diagnostic report entry.

        Supports both legacy calls:
            add_diagnostics_report(status, json_path, text_path, summary=None)

        and newer keyword-driven calls:
            add_diagnostics_report(
                job_id=1,
                status="pass",
                summary="ok",
                report_json_path="/tmp/report.json",
                report_text_path="/tmp/report.txt",
            )
        """
        conn = self._get_conn()
        cursor = conn.cursor()
        ...
        if args:
            status = args[0] if len(args) > 0 else kwargs.get('status')
            json_path = args[1] if len(args) > 1 else kwargs.get('json_path')
            text_path = args[2] if len(args) > 2 else kwargs.get('text_path')
            summary = args[3] if len(args) > 3 else kwargs.get('summary')
            job_id = kwargs.get('job_id')
        else:
            job_id = kwargs.get('job_id')
            status = kwargs.get('status') or kwargs.get('overall_status')
            json_path = kwargs.get('report_json_path') or kwargs.get('json_path')
            text_path = kwargs.get('report_text_path') or kwargs.get('text_path')
            summary = kwargs.get('summary')

        if not status or not json_path or not text_path:
            raise TypeError('status, json_path/report_json_path, and text_path/report_text_path are required')

        columns = {
            row['name']
            for row in conn.execute("PRAGMA table_info(diagnostics_reports)").fetchall()
        }

        if {'job_id', 'status'}.issubset(columns):
            cursor.execute('''
                INSERT INTO diagnostics_reports (job_id, status, summary, report_json_path, report_text_path)
                VALUES (?, ?, ?, ?, ?)
            ''', (job_id or 0, status, summary, json_path, text_path))
        else:
            cursor.execute('''
    ...
    def get_diagnostics_reports(self, limit: int = 20) -> List[Dict]:
        ...
        order_column = 'created_at' if 'created_at' in columns else 'timestamp'
        cursor.execute(f'SELECT * FROM diagnostics_reports ORDER BY {order_column} DESC LIMIT ?', (limit,))
        reports = [dict(row) for row in cursor.fetchall()]
        for report in reports:
            if 'status' not in report and 'overall_status' in report:
                report['status'] = report.get('overall_status')
            report.setdefault('job_id', None)
        return reports
