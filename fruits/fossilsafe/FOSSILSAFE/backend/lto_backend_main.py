...
from backend.tape.policy import filter_writable_tapes
...
def close_request_db_connection(exception):
    """Return database connections even if request teardown does not pop the app context promptly."""
    if db:
        db.release_connection()
...
def run_startup_validation(db_path: str):
    ...
    if os.environ.get("FOSSILSAFE_SKIP_HARDWARE_INIT", "").lower() not in ("1", "true", "yes"):
        try:
...
def _setup_request_tracing(app):
    if getattr(app, "_request_tracing_configured", False):
        return
    app._request_tracing_configured = True
...
class PreflightChecker:
    ...
    def _check_tapes_available(self, estimated_size: int) -> dict:
        try:
            ...
            available = filter_writable_tapes([
                t for t in tapes
                if t['status'] == 'available' and not t['barcode'].startswith('CLN')
            ])
...
def _sync_app_runtime_state() -> None:
    """Keep app attributes/config aligned with the mutable module globals."""
    app.db = db
    ...
    app.config['db'] = db
    app.config['verification_service'] = globals().get('verification_service') if 'verification_service' in globals() else None
...
def create_schedule():
    try:
        ...
        if source_type not in ('smb', 'local', 'nfs'):
            return _make_error_response(
                "not_implemented",
                "Source type is not implemented for schedules",
                501,
        ...
        if source_type == 'smb':
            ...
            if not source_path:
                return jsonify({'success': False, 'error': 'Selected local source has no path configured'}), 400
        elif source_type == 'nfs':
            if not (source.get('nfs_server') and source.get('nfs_export')):
                return jsonify({'success': False, 'error': 'Selected NFS source is missing server/export settings'}), 400
            ...
            if self.tape_controller.inventory_may_have_changed():
                ...
                if self.library_manager:
                    for c in self.library_manager.controllers.values():
                        try:
                            if hasattr(c, 'scan_library'):
                                scan.extend(c.scan_library('fast'))
                            else:
                                scan.extend(c.scan_barcodes())
                        except: pass
                else:
                    if hasattr(self.tape_controller, 'scan_library'):
...
def _apply_active_tape_job_state(inventory: List[Dict[str, Any]], active_jobs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    if not tape_service:
        return inventory
    return tape_service.apply_active_tape_job_state(inventory, active_jobs)
...
def initialize_app(config=None):
    ...
    if config and config.get('TESTING'):
        testing_data_dir = os.environ.get('FOSSILSAFE_DATA_DIR') or tempfile.mkdtemp(prefix='fossilsafe-test-')
        os.makedirs(testing_data_dir, exist_ok=True)
        os.environ.setdefault('FOSSILSAFE_CONFIG_PATH', os.path.join(testing_data_dir, 'config.json'))
        os.environ.setdefault('FOSSILSAFE_STATE_PATH', os.path.join(testing_data_dir, 'state.json'))
        os.environ.setdefault('FOSSILSAFE_CREDENTIAL_KEY_PATH', os.path.join(testing_data_dir, 'credential_key.bin'))
        os.environ.setdefault('FOSSILSAFE_CATALOG_BACKUP_DIR', os.path.join(testing_data_dir, 'catalog-backups'))
        os.environ.setdefault('FOSSILSAFE_DIAGNOSTICS_DIR', os.path.join(testing_data_dir, 'diagnostics'))
        os.environ.setdefault('FOSSILSAFE_HOOKS_DIR', os.path.join(testing_data_dir, 'hooks.d'))
...
def _require_job_service() -> JobService:
    if not job_service:
        raise RuntimeError('Job service unavailable')
    return job_service


def _create_internal_job(
    name: str,
    job_type: str,
    tapes: List[str],
    drive: int = 0,
    ...
    return _require_job_service().create_internal_job(
        name=name,
        job_type=job_type,
        tapes=tapes,
        drive=drive,
...
def _is_internal_job_cancelled(job_id: int) -> bool:
    if not job_service:
        return False
    return job_service.is_internal_job_cancelled(job_id)


def _mark_internal_job_cancelled(job_id: int, message: str) -> None:
    if job_service:
        job_service.mark_internal_job_cancelled(job_id, message)


def _update_job_with_log(job_id: int, status: str, message: str, level: str = 'info') -> None:
    if job_service:
        job_service.update_job_with_log(job_id, status, message, level)


def _set_job_progress(job_id: int, state: str, message: str = None, level: str = 'info') -> None:
    if job_service:
        job_service.set_job_progress(job_id, state, message, level)


def _add_job_log(job_id: int, level: str, message: str, details: str = None) -> None:
    if job_service:
        job_service.add_job_log(job_id, level, message, details)


def _cleanup_job_flag(job_id: int) -> None:
    if job_service:
...
def initialize_backend(db_path: Optional[str] = None) -> Flask:
    ...
    create_socketio()
    controllers = initialize_controllers()
    if os.environ.get("FOSSILSAFE_SKIP_HARDWARE_INIT", "").lower() in ("1", "true", "yes") or not controllers.get("hardware_available"):
...
def main():
    ...
    try:
        ...
        if controllers.get('scheduler'):
            controllers['scheduler'].stop()
        if controllers.get('autopilot'):
