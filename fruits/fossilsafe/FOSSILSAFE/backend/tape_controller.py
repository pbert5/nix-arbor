...
class TapeLibraryController:
    ...
    def __init__(
        ...
        self._mtx_unload_order = str(tape_settings.get("mtx_unload_order", "drive_slot")).lower()
        if self._mtx_unload_order not in ("drive_slot", "slot_drive"):
            self._mtx_unload_order = "drive_slot"
    ...
    def _refresh_inventory_cache(self, reason: str, mode: str = "deep") -> None:
        if self.drive_only_mode:
            return
        try:
            self._last_mtx_check = None
            inventory = self.scan_library(mode=mode)
            self._last_inventory = inventory
            self._last_inventory_at = time.time()
            self._log_event("info", "Library inventory refreshed", {"reason": reason, "count": len(inventory), "mode": mode})
        except Exception as exc:
            self._log_event("warning", "Library inventory refresh failed", {"reason": reason, "mode": mode, "error": str(exc)})
    ...
    def load_tape(self, barcode: str, drive: int = 0) -> bool:
        ...
        try:
            ...
            self._refresh_inventory_cache("pre-load", mode="fast")
            status = self.get_drive_status()
            if status.get('in_use'):
            ...
            # Find slot number for barcode using status output only.
            # `mtx status` already includes slot and volume tag data, while a
            # full inventory pass can block backup jobs for minutes on real
            # hardware before the actual load command is even attempted.
            tapes = self.scan_library(mode="fast")
            slot = None
            for tape in tapes:
            ...
            try:
                ...
                self._refresh_inventory_cache("load", mode="fast")
    ...
    def unload_tape(self, drive: int = 0, dest_slot: Optional[int] = None) -> bool:
        ...
        try:
            ...
            skip_offline_eject = bool(os.environ.get("PYTEST_CURRENT_TEST")) and str(
                os.environ.get("FOSSILSAFE_REAL_HARDWARE_TESTS", "")
            ...
            mt_success = False
            if skip_offline_eject:
                self._log_event("info", "Skipping offline/eject command during pytest run")
            elif not os.path.exists(device_path):
                self._log_event("warning", f"Drive device {device_path} unavailable; skipping offline/eject command")
            else:
                mt_wait_start = time.time()
                while time.time() - mt_wait_start < 300: # 5 minutes max wait
                    try:
                        res = self._run_mt_command('offline', drive)
                        if res.returncode == 0:
                            mt_success = True
                            break
                        err_text = (res.stderr or res.stdout or "").lower()
                        if "input/output error" in err_text or "device or resource busy" in err_text:
                            time.sleep(5)
                            continue
                        self._log_event("warning", f"Drive offline command failed: {res.stderr or res.stdout}")
                        break
            ...
            try:
                ...
                if result.returncode != 0:
                    if self._confirm_drive_unloaded(drive):
                        ...
                        return True
                    error = result.stderr or result.stdout or "Unload failed"
                    raise TapeUnloadError(error)
    ...
    def force_unload_tape(self, drive: int = 0, dest_slot: Optional[int] = None) -> bool:
        ...
        try:
            ...
            elif result.returncode != 0 and not self._confirm_drive_unloaded(drive):
                errors.append(result.stderr or result.stdout or "Unload failed")
            self._set_mounted_tape(drive, None)
            self._clear_home_slot(drive)
            if errors:
                raise Exception("; ".join(errors))
        ...
        try:
            ...
            try:
                ...
                if e.errno in (17, 107) or "Transport endpoint" in str(e):
                    ...
                    self._run_unmount_sequence(mount_point)
                    time.sleep(1)
                    os.makedirs(mount_point, exist_ok=True)
                else:
                    raise
            ...
            if device_sg and os.path.exists(device_sg):
                ...
                while time.time() - wait_start < timeout:
                    ...
                    prob_res = self.command_runner.run(
                        self._build_noninteractive_sudo_command(['sg_logs', '--page=0x38', self._get_device(drive)]),
                        timeout=10,
                        name="sg_logs_probe",
                        allow_retry=False
                    )
                    ...
                    if "busy" in err or "device or resource busy" in err:
                        ...
                        mt_res = self.command_runner.run(
                            self._build_noninteractive_sudo_command(['mt', '-f', self._get_device(drive), 'status']),
                            timeout=10,
    ...
    def _build_noninteractive_sudo_command(self, command: List[str]) -> List[str]:
        """Wrap a command in non-interactive sudo when available and needed."""
        if os.geteuid() == 0:
            return command
    ...
    def _run_unmount_sequence(self, mount_point: str) -> bool:
        """Try user-space and non-interactive unmount commands without blocking on sudo."""
        commands: List[Tuple[List[str], str]] = []
        fusermount_path = self._find_fusermount()
        if fusermount_path:
            commands.extend(
                [
                    ([fusermount_path, "-u", mount_point], "fusermount"),
                    ([fusermount_path, "-uz", mount_point], "fusermount_lazy"),
                ]
            )

        commands.extend(
            [
                (["umount", mount_point], "umount_user"),
                (["umount", "-f", mount_point], "umount_force_user"),
            ]
        )

        sudo_umount = self._build_noninteractive_sudo_command(["umount", mount_point])
        sudo_umount_force = self._build_noninteractive_sudo_command(["umount", "-f", mount_point])
        if sudo_umount != ["umount", mount_point]:
            commands.extend(
                [
                    (sudo_umount, "umount_sudo"),
                    (sudo_umount_force, "umount_force_sudo"),
                ]
            )
        ...
        for command, name in commands:
            ...
            if result.returncode == 0:
                return True
            if "not mounted" in err_msg or "not found in /etc/mtab" in err_msg:
                return True
            last_error = result.stderr or result.stdout or last_error

        raise TapeUnmountError(f"Forced unmount failed: {last_error}")
    ...
    def _parse_generation(self, barcode: str) -> str:
        ...
        if barcode and len(barcode) >= 2:
            suffix = barcode[-2:]
            gen_map = {
                'L4': 'LTO-4', 'L5': 'LTO-5', 'L6': 'LTO-6', 'L7': 'LTO-7',
