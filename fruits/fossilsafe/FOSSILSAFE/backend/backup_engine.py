...
def compute_incremental_plan(
    ...
    for entry in files:
        ...
        if not checksum:
            ...
            # File is new to this path. If identical content is already present on
            # available tape, treat it as a rename/relocation instead of re-writing.
            if has_catalog and tape_present:
                reason = PLAN_REASON_SKIPPED_PRESENT
                skipped.append({**entry, "reason": reason})
            elif has_catalog and not tape_present:
                reason = PLAN_REASON_MISSING
                to_backup.append({**entry, "reason": reason})
            else:
...
class BackupEngine:
    ...
    def _normalize_smb_path(self, smb_path: str) -> str:
        if not smb_path:
            return smb_path
        normalizer = getattr(self.smb_client, '_normalize_path', None)
        if callable(normalizer):
            return normalizer(smb_path)
        if smb_path.startswith('smb://'):
            smb_path = smb_path.replace('smb://', '//', 1)
        return smb_path.rstrip('/') if smb_path.endswith('/') else smb_path
    ...
    def _scan_source_with_hashes(
        ...
        try:
            if source_type == 'smb' and self.smb_client:
                smb_path = self._normalize_smb_path(smb_path)
        ...
        try:
            if drive == -1:
                ...
                while selected_drive is None:
                    ...
                    candidate_drives = list(self.drive_locks.keys()) or [0]
                    ...
                    # Phase 4: Short wait with repeated drive re-polling so newly-freed
                    # hardware is claimed quickly instead of waiting out the full sleep.
                    for _ in range(10):
                        random.shuffle(candidate_drives)
                        for candidate in candidate_drives:
                            lock = self.drive_locks.get(candidate)
