...
from backend.tape.policy import describe_write_block_reason
    ...
    try:
        ...
        def _do_force_unload():
            try:
                ...
                force_unload = getattr(tape_controller, 'force_unload_tape', None) or getattr(tape_controller, 'force_unload', None)
                if not force_unload:
                    raise AttributeError('Tape controller does not support force unload')
                force_unload(drive=drive, dest_slot=dest_slot)
    ...
    try:
        ...
        for b in barcodes:
            ...
            if tape and tape.get('is_cleaning_tape'):
                ...
                cleaning_tapes.append(b)
            if describe_write_block_reason(tape or {'barcode': b}):
                blocked.append(b)
        ...
        if blocked:
            return error_response(f"Tapes {', '.join(blocked)} are configured as read-only legacy media and cannot be formatted.", status_code=403)
...
def wipe_tape(barcode=None):
    ...
    try:
        ...
        if blocked:
            return error_response(f"Tapes {', '.join(blocked)} are configured as read-only legacy media and cannot be wiped.", status_code=403)
