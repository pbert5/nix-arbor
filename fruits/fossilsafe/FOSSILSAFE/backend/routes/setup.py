...
from backend.auth import get_auth_manager, require_role, is_native_2fa_enabled
...
def get_setup_tape_status():
    ...
    try:
        ...
        writable_tapes = filter_writable_tapes(tapes)
        writable_barcodes = {t.get('barcode') for t in writable_tapes if t.get('barcode')}
        skipped_tapes = [t for t in tapes if t.get('barcode') not in writable_barcodes]
        ...
        if not db:
            return success_response(data={
                'has_library': True,
                'tapes': writable_tapes,
                'count': len(writable_tapes),
                'already_initialized': False,
                'initialized_count': 0,
                'skipped_read_only': skipped_tapes,
            })
        initialized_count = 0
        for t in writable_tapes:
...
def start_tape_init():
    ...
    with _init_lock:
        ...
        inventory = ctrl.inventory()
        tapes = filter_writable_tapes([t for t in inventory if t.get('barcode') and not t.get('is_cleaning_tape')])
        
        if not tapes:
            return error_response("No writable tapes found in library")
