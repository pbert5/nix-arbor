...
def _default_restore_mount() -> dict[str, str]:
    restore_dir = Path(get_data_dir()) / 'restore'
    restore_dir.mkdir(parents=True, exist_ok=True)
    return {
        'id': 'local_default',
        'name': 'Local Recovery Folder',
        'path': str(restore_dir),
        'type': 'local',
    }


def _normalise_mount_id(prefix: str, value: str) -> str:
    safe = ''.join(char if char.isalnum() else '_' for char in value).strip('_')
    return f'{prefix}_{safe or "mount"}'


def _mount_type_for_fs(fs_type: str) -> str:
    network_fs = {'cifs', 'nfs', 'nfs4', 'smb3'}
    if fs_type in network_fs:
        return 'network'
    return 'usb' if fs_type.startswith('fuse') else 'local'


def _discover_host_mounts() -> list[dict[str, str]]:
    ...
    try:
        with open('/proc/mounts', 'r', encoding='utf-8') as mounts_file:
            for line in mounts_file:
                ...
                if fs_type in _SYSTEM_MOUNT_FS_TYPES:
                    continue
                if not any(mount_point == prefix[:-1] or mount_point.startswith(prefix) for prefix in _SYSTEM_MOUNT_PREFIXES):
                    continue
                mounts.append({
                    'id': _normalise_mount_id('mount', mount_point),
                    'name': f'Mounted Volume: {Path(mount_point).name or mount_point}',
                    'path': mount_point,
                    'type': _mount_type_for_fs(fs_type),
                })
    except OSError:
        return []
    return mounts
...
def _discover_external_mounts() -> list[dict[str, str]]:
    mounts: list[dict[str, str]] = []
    for drive in list_external_drives():
        mount_point = drive.get('mount_point')
        if not mount_point:
            continue
        label = drive.get('name') or drive.get('device') or mount_point
        mounts.append({
            'id': _normalise_mount_id('external', str(drive.get('device') or mount_point)),
            'name': f'External Drive: {label}',
...
def _get_system_mounts() -> list[dict[str, str]]:
    mounts = [_default_restore_mount()]
    seen_paths = {mounts[0]['path']}

    for mount in [*_discover_external_mounts(), *_discover_host_mounts()]:
        path = mount.get('path')
...
def get_system_mounts():
    """List restore destinations that should be offered in the UI."""
    return jsonify({
        'success': True,
        'data': {
            'mounts': _get_system_mounts()
        }
    })
