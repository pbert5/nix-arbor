...
from backend.auth import get_auth_manager, require_admin, require_auth, is_native_2fa_enabled
...
def update_sso_config():
    ...
    if not enabled:
        disabled_config = {
            'enabled': False,
            'issuer': issuer or current_config.get('issuer', ''),
            'client_id': client_id or current_config.get('client_id', ''),
            'client_secret': client_secret or current_config.get('client_secret', ''),
            'groups_claim': groups_claim,
            'role_map': json.dumps(role_map) if isinstance(role_map, dict) else role_map,
        }
        save_oidc_config(auth_mgr.db, disabled_config)
...
def login():
    ...
    try:
        ...
        native_2fa_enabled = is_native_2fa_enabled()

        # Check 2FA
        if native_2fa_enabled and user.totp_secret:
            if not totp_code:
        ...
        # Create session
        token = auth_mgr.login(username, password, has_2fa=bool(native_2fa_enabled and user.totp_secret))
        if not token:
        ...
        return success_response(data={
            ...
            'role': user.role,
            'id': user.id,
            'has_2fa': bool(native_2fa_enabled and user.totp_secret)
        })
...
def setup_2fa():
    if not is_native_2fa_enabled():
        return error_response("Native 2FA is disabled by configuration", status_code=403)
    auth_mgr = get_auth_manager()
    user = auth_mgr.get_current_user()
    if not user:
        return error_response("Not authenticated", status_code=401)
