...
INGEST_CACHED_JOB_TYPE = "ingest_cached_files"
...
def _direct_source_write_enabled(config: dict[str, Any]) -> bool:
    return bool(config.get("archive", {}).get("directSourceWrite", False))


def _is_mounted_rw(path: str) -> bool:
    """Return True if *path* is an active read-write mount."""
    result = subprocess.run(
        ["findmnt", "-n", "--output", "TARGET,OPTIONS", "--target", path],
        check=False,
        capture_output=True,
        text=True,
    )
...
def _existing_game_archive_entries(
    config: dict[str, Any],
    namespace_prefix: str,
) -> tuple[set[tuple[str, str]], set[str]]:
    existing: set[tuple[str, str]] = set()
    existing_logical_paths: set[str] = set()
    namespace = namespace_prefix.strip("/")
    prefix = f"{namespace}/"
    selected_tapes: set[str] = set()
    for entry in config.get("games", {}).get("selectedTapes", []):
        if isinstance(entry, str) and entry.strip():
            selected_tapes.add(entry.strip())
            continue
        if isinstance(entry, dict):
            barcode = entry.get("barcode")
            if isinstance(barcode, str) and barcode.strip():
                selected_tapes.add(barcode.strip())
    ...
    for file_row in db.list_files(config):
        ...
        if state not in {"missing_after_reindex", "read_error"}:
            ...
            logical_path = tape_path.removeprefix(prefix)
            existing.add((file_row["tape_barcode"], logical_path))
            if tape_path.startswith(prefix):
                existing_logical_paths.add(logical_path)

    for bundle_row in db.list_bundle_members(config):
        member_path = bundle_row["member_path"]
        logical_path = member_path.removeprefix(prefix)
        existing.add(
            (
    ...
    for job in db.list_jobs(config, limit=5000):
        ...
        if not isinstance(tape_barcode, str):
            continue
        if selected_tapes and tape_barcode not in selected_tapes:
            continue
        for staged_file in (job.get("source") or {}).get("files", []):
            ...
            if isinstance(logical_path, str):
                ...
                existing_logical_paths.add(logical_path)
    return existing, existing_logical_paths
...
def _assignment_drive_name(assignment: dict[str, Any]) -> str | None:
    for key in ("drive", "preferred_drive", "assigned_drive"):
...
def _pending_assignments(
    ...
    namespace_prefix = plan.get("namespace_prefix", "/games")
    existing_targets, existing_logical_paths = _existing_game_archive_entries(
        config,
        namespace_prefix,
    )
    ...
    for assignment in plan.get("assignments", []):
        ...
        for file_entry in assignment.get("files", []):
            ...
            if (
                logical_path in existing_logical_paths
                or (tape_barcode, logical_path) in existing_targets
            ):
                skipped_files += 1
...
def _consume_staged_file(
    ...
    if staged_file.get("direct_source"):
        return {
            "removed_files": 0,
            "removed_bytes": 0,
            "staging_dir": "",
...
def stage_games_archive(
    ...
    *plan* is the dict returned by ``_plan_game_backup`` / the
    ...
    direct_source_write = _direct_source_write_enabled(config)
    ...
    if direct_source_write:
        selected_assignments = pending_assignments
        selected_bytes = sum(
            int(assignment.get("size_bytes") or 0) for assignment in pending_assignments
        )
    ...
    for batch_index, batch in enumerate(batches, start=1):
        ...
        assigned_drives = {
            drive_name
            for drive_name in (_assignment_drive_name(assignment) for assignment in assignments)
            if drive_name is not None
        }
        if len(assigned_drives) > 1:
            raise ArchiveError(
                f"Tape {tape_barcode} was assigned to multiple drives: {sorted(assigned_drives)}"
            )
        assigned_drive = next(iter(assigned_drives), None)
                    ...
                    write_units.append(
                        {
                            ...
                            "source_path": str(source_path),
                            "logical_path": logical_path,
                            "size_bytes": logical_file["size_bytes"],
                        }
                    )
                    continue
        ...
        source_payload = {
            "kind": (
                "direct_games_archive" if direct_source_write else "staged_games_archive"
            ),
            "staging_dir": "" if direct_source_write else str(staging_dir),
            ...
            "direct_source_write": direct_source_write,
...
def promote_cached_ingest(
    ...
    *,
    ...
    """Promote a queued FUSE cached-ingest job into a write_archive job.

    This is intentionally an explicit operator step. Dropping a file into
    ``/write/inbox-cached`` only creates an ingest job; this function chooses a
    target tape and converts the cached file into the same staged shape used by
    ``write_staged_archive``.
    """
    if job["type"] != INGEST_CACHED_JOB_TYPE:
        raise ArchiveError(
            f"Job {job['id']} is type {job['type']!r}, not {INGEST_CACHED_JOB_TYPE!r}."
        )
    if job["state"] not in {"created", "queued"}:
        raise ArchiveError(
            f"Ingest job {job['id']} is {job['state']!r}; only created or queued jobs can be promoted."
        )
    if tape_barcode.strip() == "":
        raise ArchiveError("A target tape barcode is required for ingest promotion.")

    source = job.get("source") or {}
    cache_path = Path(source.get("cache_path", ""))
    relative_path = source.get("relative_path")
    if not isinstance(relative_path, str) or relative_path.strip() == "":
        raise ArchiveError(f"Ingest job {job['id']} is missing source.relative_path.")
    if not cache_path.is_file():
        raise ArchiveError(f"Cached ingest file is missing: {cache_path}")

    namespace = (namespace_prefix or (job.get("target") or {}).get("namespace_prefix") or config.get("games", {}).get("namespacePrefix", "/ingest")).strip("/")
    logical_path = str(Path("/") / relative_path.lstrip("/")).lstrip("/")
    promotion_id = str(uuid.uuid4())
    staging_dir = _cache_root(config) / "staging" / "archive-jobs" / promotion_id / tape_barcode
    staging_path = staging_dir / logical_path
    staging_path.parent.mkdir(parents=True, exist_ok=True)

    try:
        os.link(cache_path, staging_path)
    except OSError:
    ...
    source_payload = {
        "kind": "staged_fuse_ingest_archive",
        "staging_dir": str(staging_dir),
        "namespace_prefix": namespace,
        "files": [staged_file],
        "write_units": [staged_file],
    ...
    with closing(db.connect(config)) as connection:
        with connection:
            tape_id = _tape_id_from_barcode(connection, tape_barcode)
            write_job = db.create_job_with_connection(
                connection,
                WRITE_ARCHIVE_JOB_TYPE,
                state="queued",
                source=source_payload,
            ...
            connection.execute(
                ...
                WHERE job_id = ? AND cache_path = ? AND state = 'staged'
                """,
                (job["id"], str(cache_path)),
            )
            db.append_job_event(
                connection,
                write_job["id"],
                "ingest_promoted_to_write_archive",
                "Cached ingest file promoted into an archive write job.",
                {
                    "ingest_job_id": job["id"],
            ...
            db.transition_job(
                ...
                job["id"],
                "complete",
                event_type="ingest_promoted",
                message="Cached ingest file promoted into a write_archive job.",
                data={"write_job_id": write_job["id"], "tape_barcode": tape_barcode},
            )
            return {
                "ingest_job": db.get_job(connection, job["id"]),
                "write_job": db.get_job(connection, write_job["id"]),
...
def write_staged_archive(
    ...
    *,
    ...
    direct_source_write = bool(source.get("direct_source_write", False))
    ...
    if not direct_source_write and not staging_dir.is_dir():
        raise ArchiveError(
    ...
    try:
        ...
        for staged_file in write_units:
            ...
            source_path = Path(staged_file.get("source_path", ""))
            staging_path = Path(staged_file.get("staging_path", ""))
            expected_checksum: str | None = staged_file.get("checksum_sha256")
            unit_kind = staged_file.get("kind", "file")
            unit_direct_source = bool(staged_file.get("direct_source", direct_source_write))
            ...
            if final_path.exists():
                if expected_checksum is None and unit_direct_source:
                    if not source_path.is_file():
                        raise ArchiveError(
                            f"Source file is missing for direct write unit: {source_path}"
                        )
                    expected_checksum = _checksum_sha256(source_path)
    ...
    with closing(db.connect(config)) as connection:
        with connection:
            ...
            for f in written_files:
                tape_path = db._clean_catalog_path(f["tape_path"])
                parent_path, name = db._catalog_path_parts(tape_path)
                db._upsert_catalog_directory_rows(
                    connection, tape_id, tape_path, indexed_at=now
                )
                connection.execute(
                    """
                    INSERT INTO files (
                      tape_id, path, parent_path, name, is_dir,
                      logical_group, size_bytes,
...
def cleanup_staged_job_cache(
    ...
    source = job.get("source") or {}
    if source.get("direct_source_write"):
        return {
            "removed_files": 0,
            "removed_bytes": 0,
            "staging_dir": "",
        ...
        with connection:
            for row in rows:
                ...
                try:
                    ...
                    continue
                if not cache_exists and row["state"] == "staged":
                    connection.execute(
                    ...
                    continue
                try:
                    cache_is_file = cache_path.is_file()
                except PermissionError:
                    continue
                if (
                    job is not None
