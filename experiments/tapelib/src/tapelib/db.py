...
SCHEMA_STATEMENTS = [
    ...
    "CREATE INDEX IF NOT EXISTS idx_files_tape_path ON files(tape_id, path)",
...
CATALOG_INDEX_STATEMENTS = [
    "CREATE INDEX IF NOT EXISTS idx_files_tape_parent_name ON files(tape_id, parent_path, is_dir, name)",
    "CREATE INDEX IF NOT EXISTS idx_bundle_members_tape_parent_name ON bundle_members(tape_id, parent_path, name)",
]
...
def backup_database(
    ...
    *,
    ...
    source_path = database_path(config)
    default_backup_dir = Path(config.get("stateDir", "/var/lib/tapelib")) / "backups"
    backup_root = Path(
        destination_dir
        or config.get("database", {}).get("backupDir", default_backup_dir)
    )
    backup_root.mkdir(parents=True, exist_ok=True)
    timestamp = time.strftime("%Y%m%dT%H%M%SZ", time.gmtime())
    backup_path = backup_root / f"catalog-{timestamp}.sqlite"

    with closing(connect(config)) as source:
...
def initialize_database(config: dict[str, Any]) -> dict[str, Any]:
    with closing(connect(config)) as connection:
        with connection:
            ...
            _ensure_catalog_path_columns(connection)
            for statement in CATALOG_INDEX_STATEMENTS:
                connection.execute(statement)
...
def _ensure_catalog_path_columns(connection: sqlite3.Connection) -> None:
    files_columns = {
    ...
    for column, definition in {
        ...
        if column not in files_columns:
            connection.execute(f"ALTER TABLE files ADD COLUMN {column} {definition}")

    bundle_columns = {
        row["name"]
        for row in connection.execute("PRAGMA table_info(bundle_members)").fetchall()
    }
    for column, definition in {
        "parent_path": "TEXT NOT NULL DEFAULT ''",
        "name": "TEXT NOT NULL DEFAULT ''",
        ...
        if column not in bundle_columns:
            connection.execute(
                f"ALTER TABLE bundle_members ADD COLUMN {column} {definition}"
            )

    for row in connection.execute("SELECT id, path FROM files").fetchall():
        parent_path, name = _catalog_path_parts(row["path"])
        connection.execute(
            "UPDATE files SET parent_path = ?, name = ? WHERE id = ?",
            (parent_path, name, row["id"]),
        )

    now = utc_now()
    for row in connection.execute(
        "SELECT tape_id, path FROM files WHERE is_dir = 0"
    ).fetchall():
        _upsert_catalog_directory_rows(
            connection, row["tape_id"], row["path"], indexed_at=now
        )

    for row in connection.execute(
        "SELECT id, tape_id, member_path FROM bundle_members"
    ).fetchall():
        parent_path, name = _catalog_path_parts(row["member_path"])
        connection.execute(
            "UPDATE bundle_members SET parent_path = ?, name = ? WHERE id = ?",
            (parent_path, name, row["id"]),
        )
        _upsert_catalog_directory_rows(
            connection, row["tape_id"], row["member_path"], indexed_at=now
        )
...
def list_tapes(
    ...
    initialize_database(config)
    with closing(connect(config)) as connection:
        return list_tapes_with_connection(connection, include_ignored=include_ignored)


def list_tapes_with_connection(
...
def list_drives(config: dict[str, Any]) -> list[dict[str, Any]]:
    initialize_database(config)
    with closing(connect(config)) as connection:
        return list_drives_with_connection(connection)


def list_drives_with_connection(connection: sqlite3.Connection) -> list[dict[str, Any]]:
...
def get_file(
    ...
    with closing(connect(config)) as connection:
        return get_file_with_connection(
            connection, tape_barcode=tape_barcode, path=clean_path
        )


def get_file_with_connection(
    connection: sqlite3.Connection, *, tape_barcode: str, path: str
) -> dict[str, Any] | None:
    clean_path = _clean_catalog_path(path)
    row = connection.execute(
        """
        ...
        FROM files
        JOIN tapes ON tapes.id = files.tape_id
        WHERE tapes.barcode = ? AND files.path = ? AND files.is_dir = 0
        """,
...
def get_bundle_member(
    config: dict[str, Any], *, tape_barcode: str, member_path: str
) -> dict[str, Any] | None:
    initialize_database(config)
    clean_path = _clean_catalog_path(member_path)
    with closing(connect(config)) as connection:
        return get_bundle_member_with_connection(
            connection, tape_barcode=tape_barcode, member_path=clean_path
        )


def get_bundle_member_with_connection(
    connection: sqlite3.Connection, *, tape_barcode: str, member_path: str
) -> dict[str, Any] | None:
    clean_path = _clean_catalog_path(member_path)
    row = connection.execute(
        """
        ...
        WHERE tapes.barcode = ? AND bundle_members.member_path = ?
        """,
        (tape_barcode, clean_path),
    ).fetchone()
    return None if row is None else dict(row)
...
def catalog_path_has_children(
    config: dict[str, Any], *, tape_barcode: str, parent_path: str
) -> bool:
    initialize_database(config)
    with closing(connect(config)) as connection:
        return catalog_path_has_children_with_connection(
            connection, tape_barcode=tape_barcode, parent_path=parent_path
        )


def catalog_path_has_children_with_connection(
    connection: sqlite3.Connection, *, tape_barcode: str, parent_path: str
) -> bool:
    clean_parent = _clean_catalog_path(parent_path)
    for table, path_column in (
        ("files", "parent_path"),
        ("bundle_members", "parent_path"),
    ):
        row = connection.execute(
            f"""
            SELECT 1
            FROM {table}
            JOIN tapes ON tapes.id = {table}.tape_id
            WHERE tapes.barcode = ? AND {table}.{path_column} = ?
            LIMIT 1
            """,
            (tape_barcode, clean_parent),
        ).fetchone()
        if row is not None:
            return True
    return False
...
def list_catalog_child_names(
    config: dict[str, Any], *, tape_barcode: str, parent_path: str
) -> list[str]:
    initialize_database(config)
    with closing(connect(config)) as connection:
        return list_catalog_child_names_with_connection(
            connection, tape_barcode=tape_barcode, parent_path=parent_path
        )


def list_catalog_child_names_with_connection(
    connection: sqlite3.Connection, *, tape_barcode: str, parent_path: str
) -> list[str]:
    clean_parent = _clean_catalog_path(parent_path)
    names: set[str] = set()
    rows = connection.execute(
        ...
        FROM files
        JOIN tapes ON tapes.id = files.tape_id
        WHERE tapes.barcode = ? AND files.parent_path = ?
        ORDER BY files.is_dir DESC, files.name ASC
        """,
        (tape_barcode, clean_parent),
    ).fetchall()
    ...
    rows = connection.execute(
        """
        SELECT bundle_members.name AS name
        FROM bundle_members
        JOIN tapes ON tapes.id = bundle_members.tape_id
        WHERE tapes.barcode = ? AND bundle_members.parent_path = ?
        ORDER BY bundle_members.name ASC
        """,
        (tape_barcode, clean_parent),
    ).fetchall()
    names.update(str(row["name"]) for row in rows if row["name"])
...
def upsert_bundle_members_with_connection(
    ...
    *,
    ...
    for member in members:
        clean_bundle_path = _clean_catalog_path(bundle_path)
        clean_member_path = _clean_catalog_path(member["member_path"])
        parent_path, name = _catalog_path_parts(clean_member_path)
        _upsert_catalog_directory_rows(
            connection, tape_id, clean_member_path, indexed_at=indexed_at
        )
        connection.execute(
            """
            INSERT INTO bundle_members (
              tape_id, bundle_path, member_path, parent_path, name, size_bytes,
...
def list_jobs(
    ...
    initialize_database(config)
    with closing(connect(config)) as connection:
        return list_jobs_with_connection(connection, state=state, limit=limit)


def list_jobs_with_connection(
...
def list_job_events(
    ...
    initialize_database(config)
    with closing(connect(config)) as connection:
        return list_job_events_with_connection(
            connection, job_id=job_id, limit=limit
        )


def list_job_events_with_connection(
    connection: sqlite3.Connection, *, job_id: str | None = None, limit: int = 100
) -> list[dict[str, Any]]:
    if job_id is None:
        rows = connection.execute(
    ...
    with closing(connect(config)) as connection:
        with connection:
            ...
            existing_paths: set[str] = {
                row[0]
                for row in connection.execute(
                    "SELECT path FROM files WHERE tape_id = ? AND is_dir = 0",
                    (tape_id,),
                ).fetchall()
            }
            connection.execute(
                "DELETE FROM files WHERE tape_id = ? AND is_dir = 1",
                (tape_id,),
            )
            ...
            for file_path in sorted(mount_root.rglob("*")):
                ...
                parent_path, name = _catalog_path_parts(relative)
                _upsert_catalog_directory_rows(
                    connection, tape_id, relative, indexed_at=now
                )
                connection.execute(
                    """
                    INSERT INTO files (
                      tape_id, path, parent_path, name, is_dir,
                      size_bytes, mtime, state, indexed_at
                    )
                    VALUES (?, ?, ?, ?, 0, ?, ?, 'indexed', ?)
                    ON CONFLICT(tape_id, path) DO UPDATE SET
                      ...
                      indexed_at = excluded.indexed_at
                    """,
                    (tape_id, relative, parent_path, name, st.st_size, mtime, now),
                )
                indexed_count += 1
        ...
        with closing(connect(config)) as connection:
                with connection:
                        ...
                        for file_row in files:
                                ...
                                clean_path = _clean_catalog_path(path)
                                parent_path, name = _catalog_path_parts(clean_path)
                                _upsert_catalog_directory_rows(
                                        connection,
                                        tape_id,
                                        clean_path,
                                ...
                                connection.execute(
                                        ...
                                        INSERT INTO files (
                                            tape_id, path, parent_path, name, is_dir,
                                            logical_group, size_bytes, mtime,
                                            checksum_sha256, state, indexed_at
                                        ) VALUES (?, ?, ?, ?, 0, ?, ?, ?, ?, 'inventory_imported', ?)
                                        ON CONFLICT(tape_id, path) DO UPDATE SET
                                            parent_path = excluded.parent_path,
                                            name = excluded.name,
                                            is_dir = 0,
...
def _catalog_path_parts(path: str) -> tuple[str, str]:
    clean_path = _clean_catalog_path(path).rstrip("/")
    if clean_path == "":
        return "", ""
    pure = Path(clean_path)
    parent = "" if str(pure.parent) == "." else str(pure.parent)
    return parent, pure.name


def _catalog_ancestor_paths(path: str) -> list[str]:
    clean_path = _clean_catalog_path(path).rstrip("/")
    if clean_path == "" or "/" not in clean_path:
        return []
    ancestors: list[str] = []
    current = ""
    for part in clean_path.split("/")[:-1]:
        current = part if current == "" else f"{current}/{part}"
        ancestors.append(current)
    return ancestors


def _upsert_catalog_directory_rows(
    connection: sqlite3.Connection,
    tape_id: int,
    path: str,
    *,
    ...
    for directory_path in _catalog_ancestor_paths(path):
        parent_path, name = _catalog_path_parts(directory_path)
        connection.execute(
            """
            INSERT INTO files (
              tape_id, path, parent_path, name, is_dir, state, indexed_at
            )
            VALUES (?, ?, ?, ?, 1, 'directory', ?)
            ON CONFLICT(tape_id, path) DO UPDATE SET
              parent_path = CASE WHEN files.is_dir = 1 THEN excluded.parent_path ELSE files.parent_path END,
              name        = CASE WHEN files.is_dir = 1 THEN excluded.name ELSE files.name END,
              indexed_at  = CASE WHEN files.is_dir = 1 THEN excluded.indexed_at ELSE files.indexed_at END
            """,
            (tape_id, directory_path, parent_path, name, indexed_at),
        )
