...
in
{
  options.services.tapelib = {
    ...
    scheduler = {
      automaticRetrieve = lib.mkOption {
        type = lib.types.bool;
        default = false;
        description = "Allow tapelibd to load and mount tapes automatically for queued retrieve jobs.";
      };
      pollSeconds = lib.mkOption {
        type = lib.types.ints.positive;
        default = 5;
        description = "Daemon scheduler polling interval in seconds.";
      };
      unloadAfterRetrieve = lib.mkOption {
        type = lib.types.bool;
        default = false;
        description = "Unload scheduler-mounted tapes after retrieve jobs complete instead of leaving the final tape mounted.";
      };
    };
    ...
    fuse = {
      ...
      homeLink = lib.mkOption {
        type = lib.types.nullOr lib.types.str;
        default = "/home/${cfg.fuse.user}/tapelib";
        description = "Optional user-home symlink pointing at the tapelib FUSE mount.";
      };
      user = lib.mkOption {
        type = lib.types.str;
        default = "ash";
      ...
      metadataCacheSeconds = lib.mkOption {
        type = lib.types.number;
        default = 1.0;
        description = "Short in-process cache TTL for virtual FUSE metadata snapshots.";
      };
    };
    ...
    database = {
      ...
      backupDir = lib.mkOption {
        type = lib.types.str;
        default = "/var/lib/tapelib/backups";
        description = "Directory for online SQLite catalog backups.";
      };
    };
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.tmpfiles.rules = [
      ...
      "d ${cfg.database.backupDir} 2770 ${cfg.user} ${cfg.group} - -"
      ...
      "z ${cfg.stateDir}/status/database-backup.json 0660 ${cfg.user} ${cfg.group} - -"
    ...
    ++ lib.optionals (cfg.fuse.enable && cfg.fuse.homeLink != null) [
      "L+ ${cfg.fuse.homeLink} - - - - ${cfg.fuse.mountPoint}"
    ];
    ...
    systemd.services.tapelib-games-backup-run-next = {
      description = "tapelib game-library backup write runner";
      after = [ "tapelibd.service" ];
      requires = [ "tapelibd.service" ];
      ...
      serviceConfig = {
        ...
        UMask = "0007";
        WorkingDirectory = cfg.stateDir;
        ExecStart = "${lib.getExe cfg.package} --config /etc/tapelib/config.json games-backup-run-next --resume";
      };
    };
    ...
    systemd.services.tapelib-db-backup = {
      description = "tapelib SQLite catalog backup";
      path = servicePath;
      environment.TAPELIB_CONFIG_PATH = "/etc/tapelib/config.json";
      serviceConfig = {
        Type = "oneshot";
        ...
        UMask = "0007";
        WorkingDirectory = cfg.stateDir;
        ExecStart = "${lib.getExe cfg.package} --config /etc/tapelib/config.json backup-db";
      };
    };

    systemd.timers.tapelib-db-backup = {
      wantedBy = [ "timers.target" ];
      timerConfig = {
        OnBootSec = "45m";
        OnUnitActiveSec = "24h";
        Unit = "tapelib-db-backup.service";
      };
    };
