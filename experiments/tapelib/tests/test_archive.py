...
def test_stage_skips_game_file_already_cataloged_on_another_tape(tmp_path):
    config = _config(tmp_path)
    src = tmp_path / "source"
    src.mkdir()
    source_file = src / "game.bin"
    source_file.write_bytes(b"data")
    db.initialize_database(config)
    with closing(db.connect(config)) as conn:
        with conn:
            tape_id = db.get_or_create_tape(conn, "385182L5")
            conn.execute(
                """
                INSERT INTO files (tape_id, path, size_bytes, state, indexed_at)
                VALUES (?, ?, ?, ?, ?)
                """,
                (
                    tape_id,
    ...
    assert archive.stage_games_archive(config, plan) == []
...
def test_cleanup_cache_skips_permission_denied_entries(tmp_path):
    config = _config(tmp_path)
    db.initialize_database(config)
    protected = Path(config["cache"]["path"]) / "temp" / "file.bin"
    protected.parent.mkdir(parents=True, exist_ok=True)
    protected.write_bytes(b"still here")

    path_type = type(protected)
    real_unlink = path_type.unlink
    with patch.object(path_type, "unlink", autospec=True) as mock_unlink:
        ...
        mock_unlink.side_effect = side_effect
        summary = archive.cleanup_cache(config)
...
def test_promote_cached_ingest_creates_write_archive_job(tmp_path):
    config = _config(tmp_path)
    cache_file = Path(config["cache"]["path"]) / "write-inbox" / "inbox-cached" / "ready" / "new" / "game.zip"
        ...
        with conn:
            ingest_job = db.create_job_with_connection(
                ...
                source={
                    ...
                    "relative_path": "new/game.zip",
                    "cache_path": str(cache_file),
                },
                target={"namespace_prefix": "/ingest", "policy": "queue_only"},
                required_bytes=cache_file.stat().st_size,
            )
            conn.execute(
                """
                ...
                (
                    ingest_job["id"],
                    "fuse:///write/inbox-cached/new/game.zip",
                    str(cache_file),
    ...
    result = archive.promote_cached_ingest(
        config,
        ingest_job,
        tape_barcode="385182L5",
        namespace_prefix="/incoming",
    )

    assert result["ingest_job"]["state"] == "complete"
    write_job = result["write_job"]
    assert write_job["type"] == "write_archive"
    assert write_job["state"] == "queued"
    assert write_job["target"]["tape_barcode"] == "385182L5"
    assert write_job["source"]["namespace_prefix"] == "incoming"
    staged_path = Path(write_job["source"]["write_units"][0]["staging_path"])
    assert staged_path.read_bytes() == b"cached payload"

    with closing(db.connect(config)) as conn:
        states = {
            row["job_id"]: row["state"]
            for row in conn.execute(
                "SELECT job_id, state FROM cache_entries ORDER BY id"
            ).fetchall()
        }
    assert states[ingest_job["id"]] == "promoted"
    assert states[write_job["id"]] == "staged"


def test_promoted_ingest_write_archive_writes_to_tape(tmp_path):
    config = _config(tmp_path)
    mount = tmp_path / "drive0"
    _seed_tape_and_drive(config, tmp_path, "385182L5", mount)
    cache_file = Path(config["cache"]["path"]) / "write-inbox" / "inbox-cached" / "ready" / "new" / "game.zip"
    ...
    with closing(db.connect(config)) as conn:
        with conn:
            ingest_job = db.create_job_with_connection(
                ...
                target={"namespace_prefix": "/incoming", "policy": "queue_only"},
            )

    promoted = archive.promote_cached_ingest(
        config,
        ingest_job,
        tape_barcode="385182L5",
    )
    ...
    with patch.object(archive, "_find_tape_mount_path", return_value=str(mount)):
        result = archive.write_staged_archive(config, promoted["write_job"])

    assert result["state"] == "complete"
    assert (mount / "incoming" / "new" / "game.zip").read_bytes() == b"cached payload"
