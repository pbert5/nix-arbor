...
def test_run_queue_fails_without_overwriting_existing_destination(tmp_path, monkeypatch):
    ...
    assert "different contents" in result["message"]
    assert destination.read_text(encoding="utf-8") == "existing payload"
    failed = db.get_job_by_id(config, queued["job"]["id"])
    assert failed["state"] == "failed"
    assert "different contents" in failed["last_error"]


def test_run_queue_skips_existing_matching_destination(tmp_path, monkeypatch):
    config = _config(tmp_path)
    _seed_catalog(config)
    payload = "x" * 200
    source = tmp_path / "drive0" / "games" / "b.zip"
    ...
    assert result["state"] == "complete"
    assert result["copied_files"][0]["skipped_existing"] is True
    status = job_status.snapshot(config, queued["job"]["id"])
    assert status["progress_percent"] == 100
    assert status["copied_file_count"] == 1


def test_auto_run_queue_loads_mounts_and_retrieves(tmp_path, monkeypatch):
    config = _config(tmp_path)
    _seed_catalog(config)
    with closing(db.connect(config)) as connection:
        with connection:
            connection.execute(
                "UPDATE drives SET loaded_tape_id = NULL, state = 'empty' WHERE id = 'drive0'"
            )
            connection.execute(
                "UPDATE tapes SET current_location = 'slot:1', state = 'in_library' WHERE barcode = 'TAPE001L5'"
            )

    source = tmp_path / "drive0" / "games" / "a.zip"
    source.parent.mkdir(parents=True)
    source.write_text("auto payload", encoding="utf-8")
    monkeypatch.setattr(cli, "_is_mounted", lambda path: path == str(tmp_path / "drive0"))

    calls = []

    def fake_load(config_arg, barcode, drive):
        calls.append(("load", barcode, drive))
        with closing(db.connect(config_arg)) as connection:
            with connection:
                tape_id = connection.execute(
                    "SELECT id FROM tapes WHERE barcode = ?", (barcode,)
                ).fetchone()["id"]
                connection.execute(
                    "UPDATE drives SET loaded_tape_id = ?, state = 'full' WHERE id = ?",
                    (tape_id, drive),
                )
        return {"state": "complete"}

    def fake_mount(config_arg, drive, *, read_only=True):
        calls.append(("mount", drive, read_only))
        return {"state": "complete"}

    monkeypatch.setattr(cli.executor, "load_tape", fake_load)
    monkeypatch.setattr(cli.executor, "mount_ltfs", fake_mount)

    manifest = _manifest(tmp_path, ["TAPE001L5:/games/a.zip"])
    plan = cli._build_retrieve_plan(config, manifest, tmp_path / "out")
    queued = cli._queue_retrieve_plan(config, plan)

    result = cli._run_queue_once(config, auto=True)

    assert result["state"] == "complete"
    assert (tmp_path / "out" / "TAPE001L5" / "games" / "a.zip").read_text(
        encoding="utf-8"
    ) == "auto payload"
    assert calls == [("load", "TAPE001L5", "drive0"), ("mount", "drive0", True)]
    events = db.list_job_events(config, job_id=queued["job"]["id"], limit=20)
    assert any(event["event_type"] == "retrieve_loading_tape" for event in events)
    assert any(event["event_type"] == "retrieve_mounting_ltfs" for event in events)


def test_auto_run_queue_waits_without_empty_drive(tmp_path, monkeypatch):
    config = _config(tmp_path)
    _seed_catalog(config)
    ...
    assert result["ran"] is False
    assert result["state"] == "waiting_for_drive"
    assert result["blocked_tapes"] == ["TAPE001L5"]
    assert db.get_job_by_id(config, queued["job"]["id"])["state"] == "waiting_for_drive"
