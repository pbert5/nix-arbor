...
| DESKTOPTOODLE-HOST-PARITY | DONE (live evidence) | none | integration; `config/machines/desktoptoodle/` and parity evidence | `docs/desktoptoodle-parity.md` records a live `DESKTOPTOODLE PARITY: PASS`; the gate is evidence-based and does not require a specially named historical branch. |
| A1 DESKTOP TEST ACTIVATION | BLOCKED | deployment privilege + Registry bootstrap + accepted private-Ygg state | lead/integration | Activation remains blocked until physical deployment and trust prerequisites are green; parity evidence is already present and must be inspected before activation. |
