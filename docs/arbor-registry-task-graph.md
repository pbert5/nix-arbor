...
| ORBITDB-CROSS-GUEST-CONVERGENCE | DONE (local checkpoint) | DETERMINISTIC-INDEPENDENT-OPEN | registry transport + VM | bounded four-VM raw replication and multi-writer assertion passes; live relationship convergence remains separate |
| CROSS-GUEST-VM-CONVERGENCE | DONE (local checkpoint) | DETERMINISTIC-INDEPENDENT-OPEN | VM integration | VM status proves one realm/address, then root-a→child and child→root-b raw events replicate within bounded waits |
...
| RUNTIME-CONTROL-SURFACE | VALIDATION | RUNTIME-SCHEMA-PARITY; RUNTIME-DYNAMIC-KEYRING; RUNTIME-AUTHORITY-PARITY | registry runtime | packaged registryctl over durable Runtime and OrbitDBProvider; command publication uses Runtime ingestion and transport append |
| FIRST-NODE-LOSS-TRANSPORT | DONE (local checkpoint) | CROSS-GUEST-VM-CONVERGENCE | transport/VM integration | root-a offline while root-b writes, child receives the event from its existing cursor, and root-a later catches up |
| DISTRIBUTED-INDEX-LATE-EVENT | DONE (local component + VM checkpoint) | FIRST-NODE-LOSS-TRANSPORT | transport/VM integration | topology was first isolated, then the late CRDT-order insertion reproduced and fixed with append-only observation indexing |
| MULTIPLE-BOOTSTRAP-PEERS | DONE (local checkpoint) | TRANSPORT-BOOTSTRAP-CONTRACT | transport/VM integration | child and grandchild use root-a and root-b bootstrap peers; first creator loss remains live |
| LIVE-RELATIONSHIP-ACCEPTANCE | BLOCKED | RUNTIME-AUTHORITY-PARITY | registry reconciliation | live signed peer, active/standby parent, and grandchild edges |
