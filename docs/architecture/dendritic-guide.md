...
- `dendrites`, `fruits`, `users`
  what the host is selecting
- `org.network.membership`
  host network membership policy, usually `optIn = "all"` for all enabled
  inventory networks
- service-specific `org.*`
  module arguments and policy for selected dendrites
- `facts`
...
That is why a machine like `r640-0` can own storage-fabric details such as ZFS,
annex, SeaweedFS, Radicle, and deployment policy without collapsing the whole
architecture.
...
- service policy includes the needed storage and network flags
