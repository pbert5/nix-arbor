...
- Root registry discovery walks explicit nested dendrite entrypoints
  recursively.
- Dendrite entrypoints should usually import their own leaves explicitly.
- Do not build branch-local auto-loaders that make helper files active just
  because they exist.
...
- Shared attachment sets are expressed directly through explicit dendrite
  selections and `org.*` policy.
