...
The registry walks nested dendrite entrypoints recursively, but only directories
with both `<name>.nix` and `meta.nix` become selectable dendrites. Leaves and
helper folders stay inert until an entrypoint imports them.
...
- `org.network.membership` for enabled-network opt-in and opt-out policy
...
Then attach shared home behavior from user-specific
...
- service-specific policy compatibility
...
- `docs/architecture/overview.md` for repo-level shape changes
- `docs/architecture/changes.md` for implemented surface-area changes
