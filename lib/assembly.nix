...
rec {
    ...
    let
      ...
      userModules = builtins.map (
        ...
        usersLib.mkUserModule {
          extraHomeNames = lib.attrByPath [ "org" "home" "sharedModules" ] [ ] host;
          homeRegistry = registries.homes;
          user =
            inventory.users.${userName} or (throw "Host '${hostName}' references unknown user '${userName}'.");
          inherit userName;
        }
      ) (host.users or [ ]);
  ...
  resolveFruits =
    ...
    let
      ...
      requiredDendrites = lib.unique (
        lib.concatMap (
          name:
          ((fruitRegistry.${name} or (throw "Missing fruit '${name}' in dendritic registry."))
            .meta.requiresDendrites or [ ]
          )
        ) fruitNames
      );
    in
