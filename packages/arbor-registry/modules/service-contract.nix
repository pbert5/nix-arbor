...
let
  ...
  statusScript = pkgs.writeShellScript "arbor-runtime-status" ''
    ...
    if [ -x ${lib.escapeShellArg cfg.openbaoCommand} ]; then
      health=$(${lib.escapeShellArg cfg.openbaoCommand} status -format=json 2>/dev/null || true)
      if [ -n "$health" ]; then
